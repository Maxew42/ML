    ...
    def __getitem__(self, index: int):
        ...
        if self.mode != 'test':
            ...
            target['boxes'] = torch.stack(
                list((map(torch.tensor, target['boxes'])))).type(torch.float32)
...
def extractDataSetFromCOCO(dataset, imagePath):
    ...
    df['xmax'] = [row['bbox'][0]+row['bbox'][2]
                  for row in dataset['annotations']]
    df['ymax'] = [row['bbox'][1]+row['bbox'][3]
                  for row in dataset['annotations']]
    return df
        ...
        for images, image_ids in data_loader:
            ...
            for i, image in enumerate(images):
                ...
                if erase_overlaping_boxes:
                    ...
                    # We check that boxes are not overlaping too much
...
def light_evaluate(image, device, cat_to_index, model, erase_overlaping_boxes=False):
    mapping = {value: key for (key, value) in cat_to_index.items()}
